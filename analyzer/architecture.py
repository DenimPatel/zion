...
# A folder needs a few type definitions before a ratio of them means anything.
MIN_CLASSES_FOR_A = 3
RECENT_MONTHS = 3
...
def _abstractness(analysis, arch: Architecture) -> None:
    """Martin's A, and D against the instability already computed."""
    for record in analysis.files:
        if not _eligible(record):
            continue
        deps = arch.districts.get(record.district)
        if deps is None:
            deps = arch.districts[record.district] = DistrictDeps()
        deps.classes += record.classes
        deps.abstract_classes += record.abstract_classes
    any_distance = False
    for deps in arch.districts.values():
        if deps.classes < MIN_CLASSES_FOR_A:
            continue
        deps.abstractness = round(deps.abstract_classes / deps.classes, 4)
        if deps.instability is None:
            continue
        a, i = deps.abstractness, deps.instability
        deps.distance = round(abs(a + i - 1), 4)
        any_distance = True
        if a <= ZONE_LOW and i <= ZONE_LOW:
            deps.zone = "pain"
        elif a >= ZONE_HIGH and i >= ZONE_HIGH:
            deps.zone = "uselessness"
    analysis.flags.abstractness = any_distance
