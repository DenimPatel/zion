...
class ParseResult:
    ...
    # Type definitions and how many of them are abstract (architecture.py's A
    # in Martin's A/I/D). Python reads this off the AST (exact); brace
    # languages count interface/trait/protocol/abstract class keywords (medium).
    class_count: int = 0
    ...
    debt_markers: int = 0
    # The same markers with their line, marker and text, for the report and the
    # City Guide: (line, marker, text) for TODO/FIXME/HACK/XXX.
    ...
    def classes(self) -> int:
        """Alias for the abstractness denominator (deeper-signals naming)."""
        return self.class_count

    @property
    def abstract_classes(self) -> int:
        """Alias for the abstract type-definition count."""
